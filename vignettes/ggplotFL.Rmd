...
Coercion using `as.data.frame`, combined with the use of `cast` and `melt` (from the `reshape` package^[<http://cran.r-project.org/web/packages/reshape/index.html>]), provides the *FLR* user with the tools required to create a large range of `ggplot`s out of any *FLR* object.
