...
As is common in `ggplot2`, the labels can be specified directly, overwriting those included in the method. You should ignore the warning about scale for *colour* being replaced.
...
Coercion using `as.data.frame`, combined with the use of `dcast` and `melt` (from the `reshape2` package^[<http://cran.r-project.org/package=reshape2>]), provides the *FLR* user with the tools required to create a large range of `ggplot`s out of any *FLR* object.
