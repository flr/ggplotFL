.theme <- function(base_size = 12,...)
                   modifyList(theme_bw(base_size), list(...))

theme_flr<-function (size           =10,        font="",            face='plain',
...
theme_min = function (size=10,font=NA,face='plain', 
   ...
   opts(axis.text.x       = theme_text(vjust=1, hjust=0.5,   colour=axisColor, family=font, face=face, size=size),
        axis.text.y       = theme_text(hjust=1, vjust=0.5,   colour=axisColor, family=font, face=face, size=size),
        axis.title.x      = theme_text(                      colour=textColor, family=font, face=face, size=size),
        axis.title.y      = theme_text(angle=90,             colour=textColor, family=font, face=face, size=size),
        axis.line         = theme_blank(),
        axis.ticks        = theme_segment(                   colour=axisColor,                         size=0.25),
        panel.border      = theme_rect(                      colour=axisColor),
        legend.background = theme_blank(),
        legend.key        = theme_blank(),
        legend.key.size   = unit(1.5, 'lines'),
        legend.text       = theme_text(hjust=0,              colour=textColor, family=font, face=face, size=size),
        legend.title      = theme_text(hjust=0,              colour=textColor, family=font, face=face, size=size),
        panel.background  = theme_rect(fill=panelColor,      colour=NA),
        plot.background   = theme_rect(fill=backgroundColor, colour=NA),
        panel.grid.major  = theme_line(                      colour=gridColor,                         size=0.33),
        panel.grid.minor  = theme_blank(),
        strip.background  = theme_rect(fill=NA,              colour=NA),
        strip.text.x      = theme_text(hjust=0,              colour=textColor, family=font, face=face, size=size),
        strip.text.y      = theme_text(angle=-90,            colour=textColor, family=font, face=face, size=size),
        plot.title        = theme_text(hjust=0,              colour=textColor, family=font, face=face, size=size),
        plot.margin       = unit(c(0.1, 0.1, 0.1, 0.1), 'lines'))}
